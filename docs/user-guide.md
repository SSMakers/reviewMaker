...
| `날짜` | 선택 | 리뷰 작성일입니다. |
| `하이퍼링크` | 선택 | 이미 공개 URL이 있는 이미지 주소입니다. |
...
| 방식 | 설명 | 추천 상황 |
| --- | --- | --- |
| URL 우선, 없으면 파일명 | `하이퍼링크` 값이 있으면 그대로 사용하고, 없으면 `이미지파일명`으로 이미지 폴더에서 파일을 찾습니다. | 기본 추천 방식 |
...
이미지 파일명 매칭을 사용할 때는 엑셀의 `이미지파일명` 값과 실제 파일명이 정확히 같아야 합니다.
...
로컬 이미지를 자동 URL로 바꾸려면 서버가 `POST /review-images` API를 제공해야 합니다. 서버가 준비되지 않은 상태에서는 `하이퍼링크` 컬럼에 이미 공개된 이미지 URL을 넣는 기존 방식으로 사용할 수 있습니다.
