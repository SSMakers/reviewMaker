...
2. 개발자가 Slack에서 `/review-task ...`로 Codex 작업 Issue 생성 요청
3. Cloudflare Worker가 GitHub Issue를 생성하고 Slack에 Issue URL을 반환
4. 개발자가 Codex에게 Issue 처리를 요청
5. Codex가 작업 브랜치 생성
6. Codex가 코드 수정, 테스트, 버전 bump 수행
7. Codex가 PR 생성
...
15. 개발자가 Publish release를 눌러 최종 배포 승인
16. Release published 이벤트가 Pages latest metadata와 다운로드 페이지를 갱신
```

## PR에 반드시 포함할 내용

Codex는 PR 본문에 아래 내용을 포함해야 합니다.
...
Windows artifact: ReviewWriterSetup_1.0.1.exe
macOS artifact: Review_Program_1.0.1_macOS.zip
Pages metadata: latest.json version = 1.0.1
```

Windows는 PyInstaller 결과물(`Review_Program.exe`)을 Inno Setup 설치 파일(`ReviewWriterSetup_<version>.exe`)로 패키징합니다. 설치 파일은 고정 AppId를 사용하므로 같은 앱으로 업그레이드 설치됩니다. macOS는 `--onefile` 부팅 지연이 커서 PyInstaller `--onedir --windowed`로 고정 번들명(`Review_Program.app`)을 만든 뒤 zip으로 배포합니다.
...
```json
{
  ...
  "windows": {
    "url": "https://github.com/SSMakers/reviewMaker/releases/download/v1.0.1/ReviewWriterSetup_1.0.1.exe",
    "sha256": "...",
    "kind": "installer"
  },
  "macos": {
